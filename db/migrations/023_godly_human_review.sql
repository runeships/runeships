-- Flip the Godly marketing-video task to human review and append a
-- "How feedback works" section to the brief so students know the
-- review cadence up-front. Marketing video is hard to evaluate
-- algorithmically (vibe, craft, "does this make us want to download
-- the app?" need eyes), so Claude Haiku is the wrong tool here.
--
-- Originally specced as migration 016 — slot taken by the
-- seed-and-visibility migration. This is 023.
--
-- Slug stays 'onboarding-critique' because seed submissions already
-- reference this task id (same safety rule as 020/021/022).
--
-- Run AFTER 022.
...
Unlike most RuneShips tasks, this one is reviewed by the RuneShips founding team rather than by AI. Marketing video is hard to evaluate algorithmically — *feel*, *vibe*, *craft*, and *whether the video actually makes us want to download the app* are things you need eyes on. Expect feedback within **3-5 business days** of submission. You'll get a notification email when it's ready.
